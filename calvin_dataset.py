...
class PlayDataset(Dataset):
    ...
    in_features and out_features are lists of pairs. Each pair consists of (indices, offset).
    """
    ...
    def __getitem__(self, index):
        ...
        inputs = []
        for (indices, offset) in self.in_features:
            inputs.append(self.data[(start+offset):(end+offset),indices])
        inputs = torch.from_numpy(np.concatenate(inputs, axis=1))
        outputs = []
        for (indices, offset) in self.out_features:
            outputs.append(self.data[(start+offset):(end+offset),indices])
