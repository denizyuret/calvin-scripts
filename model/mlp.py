...
def train(pre = '../data/D', batch_size=128, hidden=[1024,1024], features=range(1,74), window=32, name='001', max_epochs=10, dropout=0.5, weight_decay=0.01, lr=0.001):
    global trndict, valdict, mlp
    torch.set_float32_matmul_precision('medium')
    if pre not in trndict:
        trn1 = calvindataset1(prefix = pre + '-training',   features=features, window=window)
        val1 = calvindataset1(prefix = pre + '-validation', features=features, window=window)
